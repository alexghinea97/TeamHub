...
{
    public class Fragment_in_review : Android.Support.V4.App.Fragment
    {
        ...
        public void AfiseazaTask()
        {
            ...
            if (conn.State == ConnectionState.Closed)
            {
                ...
                {
                    MySqlCommand getTasks = new MySqlCommand("SELECT task_name FROM THTasks where id_team = " + Fragment_projects.idTeamSelected + " AND status = 2;", conn);
                ...
                {
                    MySqlCommand getTasks = new MySqlCommand("SELECT task_name FROM THTasks where id_member = " + MainActivity.user_id + " AND status=2;", conn);
                    MySqlDataAdapter adapter = new MySqlDataAdapter(getTasks);
                    adapter.Fill(data);
                    String numeTask;
                    foreach (DataRow row in data.Rows)
        ...
        private void getIdTask()
        {
            ...
            if (connDel.State == ConnectionState.Closed)
            {
                ...
                if (Fragments.Fragment_projects.idTeamSelected != -1)
                {
                    MySqlCommand getTasks = new MySqlCommand("SELECT id_task FROM THTasks where id_team = " + Fragment_projects.idTeamSelected + " AND status=2;", connDel);
                    MySqlDataAdapter adapter = new MySqlDataAdapter(getTasks);
                ...
                {
                    MySqlCommand getTasks = new MySqlCommand("SELECT id_task FROM THTasks where id_member = " + MainActivity.user_id + " AND status=2;", connDel);
                    MySqlDataAdapter adapter = new MySqlDataAdapter(getTasks);
                    adapter.Fill(dataDel);
                    foreach (DataRow row in dataDel.Rows)
                    {
        ...
        {
            View view = inflater.Inflate(Resource.Layout.Fragment_in_review_layout, container, false);
